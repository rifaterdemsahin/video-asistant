...
| Projected cost at 10x scale | AI model API usage (pay-per-token via existing keys) |
| Most expensive active component | None yet (all free tier / existing paid API keys in `dp-kv-deliverypilot`) |
| Next likely paid switch | AI model token spend once automated workflows run regularly |
