...
- **Open Links in Chrome** — Whenever the user says **"open"** or **"launch"** something with a URL — a link, local preview, deployed page, or an AI Chat Link (see `2_Environment/tools.md`) — always open it in Google Chrome, never the OS default browser. Use `open -a "Google Chrome" <url>`.
